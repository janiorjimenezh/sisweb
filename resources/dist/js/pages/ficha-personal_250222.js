function get_datos_dni(e){
    ...
    $('#frmins-personales #fictipocolegio').val(e.vdata['tipocoleg']);
    $('#frmins-personales #ficanioculcolegio').val(e.vdata['aniocoleg']);
    $('#frmins-personales #ficbdepartamentocoleg').val(e.vdata['coddepartamentocoleg']);
    $('#frmins-personales #ficbprovinciacoleg').html(e.provinciascol);
    $('#frmins-personales #ficbprovinciacoleg').val(e.vdata['codprovinciacoleg']);
    $('#frmins-personales #ficbdistritocoleg').html(e.distritoscol);
    $('#frmins-personales #ficbdistritocoleg').val(e.vdata['coddistritocoleg']);

    estcolextranj = (e.vdata['extransecundaria'] == 'SI') ? true : false;
    $('#frmins-personales #check_extranjero').prop('checked', estcolextranj);
    $('#frmins-personales #fitxtdireccion_extranjero').val(e.vdata['extradireccion']);
    $('#check_extranjero').change();
    ...
    $('#frmins-personales #ficblenguas').val(e.vdata['lengua']);
    $('#frmins-personales #ficbotrlenguas').val(e.vdata['otlengua']);
...
$('#frmins-personales #fibtnvalida-dni').on('click', function() {
    ...
    $.ajax({
        url: base_url + 'persona/fn_get_datos_alumno_por_dni',
        type: 'post',
        dataType: 'json',
        data: {
            fitxtdni: dni
...
$('#frmins-personales input,select').change(function() {
    ...
    // UBIGEO COLEGIO SECUNDARIO
    if ($(this).attr('id') == "ficbdepartamentocoleg") {
        $('#frmins-personales #ficbprovinciacoleg').html("<option value='0'>Sin opciones</option>");
        $('#frmins-personales #ficbdistritocoleg').html("<option value='0'>Sin opciones</option>");
        var coddepa = $(this).val();
        if (coddepa == '0') return;
        $.ajax({
            url: base_url + 'ubigeo/fn_provincia_x_departamento',
            ...
            success: function(e) {
                $('#frmins-personales #ficbprovinciacoleg').html(e.vdata);
            },
            error: function(jqXHR, exception) {
                var msgf = errorAjax(jqXHR, exception, 'text');
                $('#frmins-personales #ficbprovinciacoleg').html("<option value='0'>" + msgf + "</option>");
            }
        });
    } else if ($(this).attr('id') == "ficbprovinciacoleg") {
        $('#frmins-personales #ficbdistritocoleg').html("<option value='0'>Sin opciones</option>");
        var codprov = $(this).val();
        if (codprov == '0') return;
        $.ajax({
            ...
            success: function(e) {
                $('#frmins-personales #ficbdistritocoleg').html(e.vdata);
            },
            error: function(jqXHR, exception) {
                var msgf = errorAjax(jqXHR, exception, 'text');
                $('#frmins-personales #ficbdistritocoleg').html("<option value='0'>" + msgf + "</option>");
            }
        });
        return false;
    }
...
$("#frmins-personales").submit(function(event) {
    ...
    $('#divdata_contacto .vw_fcb_class_rowitem').each(function(index, el) {
        var itemd = {};
        itemd['fictxtcodigo'] = $(this).find('[name=fictxtcodigo]').val();
        itemd['fictxttiporela'] = $(this).find('[name=fictxttiporela]').val();
        itemd['fictxtapenomcontac'] = $(this).find('[name=fictxtapenomcontac]').val();
        itemd['fictxtnumerocontac'] = $(this).find('[name=fictxtnumerocontac]').val();
        if (itemd['fictxttiporela'] !== "" && itemd['fictxtapenomcontac'] !== "" && itemd['fictxtnumerocontac'] !== "") {
            arrcontact.push(itemd);
        }
        
    })

    acontac= JSON.stringify(arrcontact);
    fdata=$(this).serializeArray();
    fdata.push({name: 'items', value: acontac});
    console.log("acontac", acontac);
    fdata.push({name: 'check_extranjero', value: check_extranjero});

    $('#divboxhistorial').append('<div id="divoverlay" class="overlay"><i class="fas fa-spinner fa-pulse fa-3x"></i></div>');
    urlpersonales = base_url + 'persona/fn_update_datos_personales';
    if ($(this).data('action') == 'insert') urlpersonales = base_url + 'persona/fn_insert_datos_personales';
    $.ajax({
        url: urlpersonales,
        type: 'post',
        dataType: 'json',
        ...
        success: function(e) {
            ...
            if (e.status == false) {
                ...
                location.href = base_url + "admision/ficha-personal?dni="+ $('#frmins-personales #fitxtdni').val() + sb;
                Swal.fire({
                    type: 'success',
...
function fn_valida_reniec_admision(documento) {
    var dni = documento;
    $('#divboxhistorial').append('<div id="divoverlay" class="overlay bg-white d-flex justify-content-center align-items-center"><i class="fas fa-spinner fa-pulse fa-3x"></i></div>');
    if ((dni.length!=8) || (!$.isNumeric(dni))){
        Swal.fire({
            type: 'error',
            title: "DNI incorrecto",
