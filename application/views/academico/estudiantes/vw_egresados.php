...
<link rel="stylesheet" type="text/css" href="https://cdn.datatables.net/v/bs4/dt-1.13.7/b-2.1.1/sl-1.3.4/datatables.min.css"/>
    ...
    <div id="divboxhistorial" class="card">
      <div class="card-header p-2">
        <ul class="nav nav-pills">
          <li class="nav-item">
            <a class="nav-link active" href="#egresados" data-estado='EGRESADO' id="insTabegresados" data-modalidad='egresados' data-toggle="tab">
              <u>Egresados</u>
            </a>
          </li>
          <li class="nav-item">
            <a class="nav-link" href="#culminado" data-estado='CULMINADO' id="insTabculminado" data-modalidad='culminado' data-toggle="tab">
              <u>Culminados</u>
            </a>
          </li>
          <li class="nav-item">
            <a class="nav-link" href="#cuestionarios" data-estado='C' id="insTabcuestionarios" data-modalidad='cuestionarios' data-toggle="tab">
              <u>Cuestionario</u>
            </a>
          </li>
          ...
          <li id="tabli-aperturafile" class="nav-item">
            <a class="nav-link" href="#reportes" data-estado='R' id="insTabreportes" data-modalidad='reportes' data-toggle="tab">
              <u>Reportes</u>
            </a>
          </li>
      ...
      <div class="card-body p-2">
        <div class="tab-content">
          <div class="active tab-pane pt-3" id="egresados">
            ...
            <form id="frm-filtro-inscritos" name="frm-filtro-inscritos" action="<?php echo $vbaseurl ?>academico/estudiantes/filtro/egresados" method="post" accept-charset='utf-8'>
              
              <div class="row my-2">
                <input type="hidden" name="fbus-modalidad" id="fbus-modalidad" value="%">
                <input type="hidden" name="fbus-estado" id="fbus-estado" value="EGRESADO">
          ...
          <div class="tab-pane" id="cuestionarios">
              
              <div class="card-body p-2">
                <form id="vw_cue_frmcuestionario" name="vw_cue_frmcuestionario"  method="post" accept-charset='utf-8'>
                  <div class="row">
                    
                      <div class="form-group has-float-label col-12 col-sm-2 col-md-2">
                        <select name="vw_cue_txtsede" id="vw_cue_txtsede" class="form-control form-control-sm">
                          <option value="%">Todos</option>
                          <?php 
                            foreach ($sedes as $filial) {
                              $select=($_SESSION['userActivo']->idsede==$filial->id) ? "selected":"";
                        ...
                        <label for="vw_cue_txtsede"><i class="far fa-building"></i> Filial</label>
                      </div>
                      <div class="form-group has-float-label col-12 col-sm-3 col-md-3">
                        <select class="form-control form-control-sm" id="vw_cue_txtcarrera" name="vw_cue_txtcarrera" placeholder="Programa">
                          <option value="%">Todos</option>
                          <?php foreach ($carreras as $carrera) {?>
                          <option value="<?php echo $carrera->codcarrera ?>" data-sigla="<?php echo $carrera->sigla ?>"><?php echo $carrera->nombre ?></option>
                          <?php } ?>
                        </select>
                        <label for="vw_cue_txtcarrera"> Programa</label>
                      </div>
                      <div class="form-group has-float-label col-12 col-sm-6 col-md-4">
                        <input autocomplete="off"  class="form-control form-control-sm text-uppercase" id="vw_cue_txtbuscar" name="vw_cue_txtbuscar" type="text" placeholder="Carné o Apellidos y nombres"   />
                        <label for="vw_cue_txtbuscar">Carné o Apellidos y nombres</label>
                      </div>
                      <div class="col-4 col-sm-3 col-md-1">
                        <a id="vw_cue_btnbuscar" href="#" class="btn btn-primary btn-block btn-sm"><i class="fas fa-search"></i></a>
                      </div>
                ...
                <div class="col-12 px-0 pt-2 table-responsive">
                  ...
                  <table id="tbmt_dtEncuestados" class="tbdatatable table table-sm table-hover table-bordered table-condensed display nowrap" style="width:100%">
                    <thead>
                        <tr class="bg-lightgray">
                            <th>N°</th>
                            <th>DOC.</th>    
                            ...
                            <th>Factor</th>
                            <th>Beca</th>
                            <th>Conoc.Técnicos</th>
                            <th>Ideas</th>
                            <th>Claridad</th>
                            <th>Herr.Inform</th>
                            <th>Trab.Equipo</th>
                            <th>Leng.Extranjera</th>
                            <th>Toma Decisiones</th>
                            <th>Liderazgo</th>
                            <th>Conoc.Unidad</th>
                            <th>Metodologías</th>
                            <th>Material</th>
                            <th>Exp.Formativas</th>
                            <th>Convenios</th>
                            <th>Bolsa Trabajo</th>
                            <th>Ocupación</th>
                            <th>Vinculo</th>
                            <th>EntidadLaboral</th>
                            <th>LaboraActual</th>
                            <th>Empresa</th>
                            <th>Remuneración</th>
                            <th>Satisfacción</th>
...
<script type="text/javascript" src="https://cdn.datatables.net/v/bs4/dt-1.13.7/b-2.1.1/sl-1.3.4/datatables.min.js"></script>
...
<script>
  jsmodalidad="<?php echo $modalidadActiva ?>";
      $(document).ready(function() {
        ...
        var table2 = $('#tbmt_dtEncuestados').addClass( 'nowrap' )
          .dataTable( {
            "responsive": true,
            "pageLength": 50,
            "lengthChange": false,
    ...
    $('.nav-pills a').on('shown.bs.tab', function(event){
      $("#fbus-estado").val($(event.target).data('estado'));
      ...
      //   $("#divres-alumno").show();
      // }
        valnavtap = $(event.target).data('modalidad');
        ...
        if (($("#fbus-estado").val()=="EGRESADO") || ($("#fbus-estado").val()=="CULMINADO")){
          $("#frm-filtro-inscritos").submit();
        }
          ...
          success: function(e) {
              ...
              if (e.status == false) {
                  ...
                  $.each(e.vdata, function(index, v) {
                    ...
                    grupo = "<div title='"+v['carrera']+"'>" + v['carsigla'] + "</div>";

                    var fila = tbinscritos.row.add([index + 1, v['periodo'],  grupo, vcarnet, icongen+" "+nombres,  dropdown_estado, dropdown_acc + " " + imprficha + " " + btnmatric]).node();
  ...
  $(".btn-excel").click(function(e) {
    ...
    if ((valnavtap == "") || (valnavtap == "egresados")) {
      urlnavtap = "egresados";
    } else {
      urlnavtap = valnavtap;
    }

    var enlaceurl = "admision/"+urlnavtap+"/excel";

    var url=base_url + enlaceurl + '?cp=' + $("#fbus-periodo").val() + '&cc=' + $("#fbus-carrera").val()  + '&tn=' +  $("#fbus-turno").val()  + '&cl=' +  $("#fbus-ciclo").val() + '&ccp=' + $("#fbus-campania").val() + '&cs=' + $("#fbus-seccion").val() +'&ap=' + $("#fbus-txtbuscar").val() +'&mdld=' + $('#fbus-estado').val() +'&sede=' + $('#fbus-sede').val();
    var llenos=0;
    if ($("#fbus-txtbuscar").val()!=="") llenos=llenos + 2;
    if ($("#fbus-periodo").val()!=="%") llenos++;
    if ($("#fbus-campania").val()!=="%") llenos++;
    ...
    if ($("#fbus-turno").val()!=="%") llenos++;
    if ($("#fbus-ciclo").val()!=="%") llenos++;
    if ($("#fbus-sede").val()!=="%") llenos=llenos + 2;
  ...
  $("#vw_cue_btnbuscar").click(function(e) {
      e.preventDefault();
      // vw_cue_txtsede
      // vw_cue_txtcarrera
      // vw_cue_txtbuscar
  ...
  function fn_data_encuestas_egresados() {
      
      $('#divboxhistorial').append('<div id="divoverlay" class="overlay"><i class="fas fa-spinner fa-pulse fa-3x"></i></div>');
      tbencuestas = $('#tbmt_dtEncuestados').DataTable();
      //tbencuestas.clear();
      // $('#vw_mt_divmensaje').show();
      // $("#vw_mt_spanmensaje").hide();
      $.ajax({
          url: base_url + "encuesta/filtro/egresados-lista",
          type: 'post',
          dataType: 'json',
          data: $("#vw_cue_frmcuestionario").serialize(),
          success: function(e) {
              $('#divboxhistorial #divoverlay').remove();
              tbencuestas.clear();
              if (e.status == false) {
                  ...
                  $.each(e.vdata, function(index, v) {
                    ...
                    icongen = (v['genero'] =='MASCULINO') ? '<i class="fas fa-male fa-lg text-primary"></i>':'<i class="fas fa-female  fa-lg text-danger"></i>'
                    vdocument = "<div class='cell-doc' title='"+v['tipodoc']+"'>"+v['nrodoc']+"</div>";
                    nombres = "<div class='cell-egresado d-inline-block'>"+v['egresado']+"</div>";

                    var fila = tbencuestas.row.add([index + 1, vdocument,v['sede'], icongen+" "+nombres, v['fechanac'], v['lugarnac'], v['residencia'], v['celular'], v['correo'], v['ruc'], v['lengua'], v['estadocivil'], v['carrera'], v['nivel'], v['anioegreso'], v['aniotitulo'], v['motivo'], v['factor'], v['beca'], v['conoctecnico'], v['ideas'], v['claridad'], v['herraminform'], v['trabequipo'], v['lengextranj'], v['tomadecis'], v['liderazgo'], v['conocunidad'], v['metodologias'], v['material'], v['efsrt'], v['convenios'], v['bolsatrab'], v['ocupacion'], v['vinculo'], v['entidadlaboral'], v['empleoactual'], v['nomempresa'], v['remuneracion'], v['satisfacion'], v['favorecen']]).node();

                    $(fila).attr('class', "cfilaenc");
                  })
