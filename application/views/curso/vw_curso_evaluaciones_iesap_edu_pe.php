                                        ...
                                        if ($evaluacion->indicador==$indicador->codigo){
                                            if ($evaluacion->abrevia!="RC"){
                                                ...
                                                if ($tipo=="M"){
                                                    ...
                                                    if (($editarUnidEval[$indicador->norden]==false) || (getPermitido("128")=="SI")){
                                                    ...
                                                    echo "<td class='cell text-right' >
                                                        <span id='{$aidmiembro}{$indicador->codigo}{$evaluacion->nombre_calculo}' data-indicador='{$evaluacion->indicador}' data-peso='{$evaluacion->peso}' data-valor='{$valor}' data-edit='0' data-ideval='{$evaluacion->evaluacion}' data-idnota='{$aid}' data-miembro='{$aidmiembro}' data-ntsaved='{$anota}'  class='spnnota $colorbtn'>".str_pad($anota, 2, "0", STR_PAD_LEFT)."</span>
                                                        </td>";
                                                }
                                            }
                                        }
...
$('#btnguardareg').click(function(event) {
    ...
    $('#tbasistencia td .spnnota').each(function() {
        var isedit = $(this).data("edit");
        var idnota = $(this).data("idnota");
        var fcha = $(this).data("fecha");
        var nota = Number($(this).text());
        var idmiembro = $(this).data("miembro");
        var idevh = $(this).data("ideval");
        //dataString = {fecha: fcha, accion: accn ,idmiembro: idacu};
...
function promediar(vidmiembro){
    if (jsmetodo=="PFGN"){
        ...
        for (i = 0; i < vindicadores.length ; i++) {
            ...
            var n1=$("#" + vidmiembro + ind + "N1").val();
            var n2=$("#" + vidmiembro + ind + "N2").val();
            var n3=$("#" + vidmiembro + ind + "N3").val();
        ...
        if (nindicadores==0){
            ...
            pi=Math.round(suma/nindicadores);
        }
        ...
        for (i = 0; i < vindicadores.length ; i++) {
            ...
            pindicador= Math.round((Number(n1) * 0.3) + (Number(n2) * 0.3) + (Number(n3) * 0.4));
            $("#" + vidmiembro + ind + "C1").html("<span>" + pindicador + "</span>");
            peso=$("#" + vidmiembro + ind + "C1").data('peso');
            notaGuardada=$("#" + vidmiembro + ind + "C1").data('valor');
            if (Number(notaGuardada)!=pindicador){
                $("#" + vidmiembro + ind + "C1").data('edit','1');
            }
            porc_indicador=pindicador * Number(peso);
