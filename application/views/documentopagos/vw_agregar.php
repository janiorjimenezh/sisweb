...
if (isset($mat->emision))  $vemision=$mat->emision;
$vimporte = (isset($mat->importe)) ? $mat->importe : "0.00";
// if (isset($mat->importe))  { $vimporte=$mat->importe; }else{ $vimporte="0.00"; };
if (isset($mat->peso))  $vpeso=$mat->peso;
if (isset($mat->ruta))  $vruta=$mat->ruta;
if (isset($mat->estado))  $vestado=$mat->estado;
...
<div class="content-wrapper">
    ...
    <section id="s-cargado" class="content">
        <div id="vw_pw_bt_ad_div_principal" class="card">
            <div class="card-body">
                <form id="vw_pw_bt_ad_form_addDWeb" action="" method="post" accept-charset="utf-8">
                    <div class="row mt-2">
                        ...
                        <div class="form-group has-float-label col-6 col-sm-4">
                            <input data-currentvalue='' autocomplete="off" class="form-control" id="vw_pw_bt_ad_fictxtemision" name="vw_pw_bt_ad_fictxtemision" type="date" placeholder="" value="<?php echo $vemision ?>" />
                            <label for="vw_pw_bt_ad_fictxtemision">Fecha emisión</label>
                        </div>
                        <div class="form-group has-float-label col-6 col-sm-4">
                            <input data-currentvalue='' autocomplete="off" class="form-control" id="vw_pw_bt_ad_fictxtimporte" name="vw_pw_bt_ad_fictxtimporte" type="number" step="0.01" placeholder="Importe" value="<?php echo $vimporte ?>" />
                            <label for="vw_pw_bt_ad_fictxtimporte">Importe</label>
                        </div>
                        <div class="form-group has-float-label col-12 col-sm-4">
                            <select name="vw_pw_bt_ad_fictxtestado" id="vw_pw_bt_ad_fictxtestado" class="form-control">
                                ...
                                <option <?php echo ($vestado == "VENCIDO") ? "selected" : "" ?> value="VENCIDO">VENCIDO</option>
                                <option <?php echo ($vestado == "PAGADO") ? "selected" : "" ?> value="PAGADO">PAGADO</option>
                                <option <?php echo ($vestado == "PENDIENTE") ? "selected" : "" ?> value="PENDIENTE">PENDIENTE</option>
                                <option <?php echo ($vestado == "ANULADO") ? "selected" : "" ?> value="ANULADO">ANULADO</option>
                            </select>
                        ...
                        <div class="col-12">
                            <a type="button" href="<?php echo $vbaseurl ?>documentos/pagos" class="btn btn-danger btn-md float-left" >
...
$("#vw_pw_bt_ad_btn_guardar").click(function(event) {
    ...
    $.ajax({
        url: base_url + 'documentos_pago/fn_guardar',
        type: 'POST',
        data: fdata,
        dataType: 'json',
        success: function(e) {
