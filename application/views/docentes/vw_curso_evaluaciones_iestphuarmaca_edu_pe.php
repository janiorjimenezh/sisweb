            ...
            <div class="card-body px-2">
                ...
                <div class="has-float-label col-12 p-0 mb-2">
                        <select data-currentvalue='' class="form-control text-bold text-primary" id="cbindicador" name="cbindicador" placeholder="Indicador" required >
                            ...
                            <?php } ?>
                            <option value="-1">Todos los indicadores</option>
                            <option class="d-block d-sm-none" value="0">Notas Finales</option>
                        </select>
                ...
                <table class="table-registro" id="tbasistencia" role="table">
                    <thead role="rowgroup">
                        <tr role="row">
                            ...
                            <?php
                                foreach ($indicadores as $indicador) {
                                        echo "<td colspan='5' class='text-center text-bold ind-head$indicador->codigo'>INDICADOR $indicador->norden</td>";
                                }
                            ?>
                        ...
                        <tr role="row">
                            <th class="cell" data-indicador='-1' role="columnheader"><span class="d-none d-sm-block">CARNÉ</span></th>
                            <th class="cell" data-indicador='-1' role="columnheader">ALUMNO</th>
                            <?php
                                foreach ($indicadores as $indicador) {
                                    foreach ($evaluaciones as $evaluacion) {
                                        if ($evaluacion->indicador==$indicador->codigo){
                                            ...
                                            <span class='mt-1 rotar $color'>".$evaluacion->abrevia.$indicador->norden."</span></th>";
                                        }
                                    }
                                    echo "<th  class='cell ' data-indicador='$indicador->codigo' data-toggle='tooltip' data-placement='top' title='Promedio de Indicador $indicador->norden'>
                                            <span class='mt-1 rotar $color'>PI".$indicador->norden."</span></th>";
                                }
                            ?>
                                            ...
                                            foreach ($indicadores as $indicador) {
                                                foreach ($evaluaciones as $evaluacion) {
                                                    if ($evaluacion->indicador==$indicador->codigo){
                                                        ...
                                                        if ($valor>=13) $colorbtn="text-primary";
                                                        ...
                                                        if ($tipo=="M"){
                                                            $aid= $notas[$aidmiembro]['eval'][$evaluacion->indicador][$evaluacion->abrevia]['idnota'] ;
                                                             echo "<td class='cellnota' >
                                                                <input step='0.1' id='".$aidmiembro.$indicador->codigo.$evaluacion->abrevia."' type='number' data-indicador='".$evaluacion->indicador."' data-valor='".$valor."' max='20' min='0' data-edit='0' data-ideval='".$evaluacion->evaluacion."' data-idnota='".$aid."' data-miembro='".$aidmiembro."' class='txtnotadecimal ".$colorbtn."' data-ntsaved='".$anota."' value='".$anota."'>
                                                            </td>";
                                                        }
                                                        else{
                                                            //var_dump($anota);
                                                ...
                                                $valor= $notas[$aidmiembro]['eval'][$indicador->codigo]['PI']['nota'] ;

                                                $colorbtn="text-danger";
                                                if ($valor>=13) $colorbtn="text-primary";
                                                echo "<td class='cell text-right' >
                                                    <span id='".$aidmiembro.$indicador->codigo."PI' class='$colorbtn'>$valor</span>
                                                </td>";
...
<script>
        ...
        if (nindicador=='-1'){
          ...
          //alert(ancho);
            ...
            $('.table-registro th').each(function () {
                var ind = Number($(this).data("indicador"));
                $('.ind-head' + ind).hide();
                if (ind>=colmin){
    ...
    $("#switchpf").change(function(event) {
        ...
        if (chk==true){
            //$("table tr td:nth-child(" + (i) + "), table tr th:nth-child(" + (i) + ")").show();
            $('.table-registro .cellfinal').show();
        }
        else{
            $('.table-registro .cellfinal').hide();
...
function promediar(vidmiembro){
    ...
    for (i = 0; i < vindicadores.length ; i++) {
        ...
        var ep=Number($("#" + vidmiembro + ind + "EP").val());
        //eps =  Number(ep) + eps;
        var ta=Number($("#" + vidmiembro + ind + "TA").val());
        //tas = Number(ta) + tas;
        var ef=Number($("#" + vidmiembro + ind + "EF").val());

        var rci=Number($("#" + vidmiembro + ind + "RC").val());

        var pfsr=((ep * 3) + (ta * 4) + (ef * 3))/10;
        
        var pfr3=pfsr.toFixed(3);
        var pfr2=Math.round(Math.round(pfr3 * 1000) / 10) / 100 
        var pfr1=Math.round( pfr2 * 10 ) / 10
        var pf=Math.round(pfr1);
        pf=(rci>pf)?rci:pf;
        span=$("#" + vidmiembro + ind + "PI");
        span.html(pf);
        if (pf >= 13) {
    ...
    if (i==0) i=1;
    var prf=pfs/i;
    var pfi2=Math.round(Math.round(prf * 1000) / 10) / 100 
    var pfi1=Math.round( pfi2 * 10 ) / 10
    var pfi=Math.round(pfi1);

    var ri=($("#" + vidmiembro +  "RC").val() * 1);
    var pif= ((pfi>ri) ? pfi : ri);

    
    spanpi=$("#" + vidmiembro + "PI");
    spanpi.html(pfi)
    if (pfi >= 13) {
        spanpi.removeClass('text-danger');
        spanpi.addClass('text-primary');
    } else {
        spanpi.removeClass('text-primary');
        spanpi.addClass('text-danger');
    }
