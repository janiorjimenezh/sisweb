		...
		<?php 
			...
			foreach ($miembros as $miembro) {
					...
					echo '<tr role="row">';
						...
						$tind=($tind>$idfin) ? $idfin : $tind;
						for ($i=$idini; $i <$tind ; $i++) { 
						...
						//foreach ($indicadores as $key => $indicador) {
								...
								$n1=floatval($alumno[$miembro->idmiembro]['eval'][$indicador->codigo]['PC']['nota']);  
								$color = ($n1>=12.5) ? "negro":"rojo";
								echo "<td class='celda ".$color."'>".str_pad($n1, 2, "0", STR_PAD_LEFT)."</td>";  

								$t1=floatval($alumno[$miembro->idmiembro]['eval'][$indicador->codigo]['TA']['nota']); 
								$color = ($t1>=12.5) ? "negro":"rojo";
								echo "<td class='celda ".$color."'>".str_pad($t1, 2, "0", STR_PAD_LEFT)."</td>"; 


								$e1=floatval($alumno[$miembro->idmiembro]['eval'][$indicador->codigo]['EI']['nota']); 
