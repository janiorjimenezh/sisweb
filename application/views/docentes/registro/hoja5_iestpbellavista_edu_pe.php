...
<div style="width: 47%; float: left;">
	<table class="tbnotas"  autosize="1"  cellpadding="0">
		...
		<tr >
			...
			<th align="center" class="head rotar90">CC</th>
			<th align="center" class="head rotar90">PC</th>
			<th align="center" class="head rotar90">AP</th>
			...
			foreach ($miembros as $miembro) {
					...
					echo '<tr role="row">';
						...
						//foreach ($indicadores as $key => $indicador) {
							...
							if ($nind<5){
								$n1=floatval($alumno[$miembro->idmiembro]['eval'][$indicador->codigo]['CC']['nota']);  
								$color = ($n1>=12.5) ? "negro":"rojo";
								echo "<td class='celda ".$color."'>".str_pad($n1, 2, "0", STR_PAD_LEFT)."</td>";  

								$t1=floatval($alumno[$miembro->idmiembro]['eval'][$indicador->codigo]['PC']['nota']); 
								$color = ($t1>=12.5) ? "negro":"rojo";
								echo "<td class='celda ".$color."'>".str_pad($t1, 2, "0", STR_PAD_LEFT)."</td>"; 


								$e1=floatval($alumno[$miembro->idmiembro]['eval'][$indicador->codigo]['AP']['nota']); 
								$color = ($e1>=12.5) ? "negro":"rojo";
								echo "<td class='celda ".$color."'>".str_pad($e1, 2, "0", STR_PAD_LEFT)."</td>"; 

								$r1=$alumno[$miembro->idmiembro]['eval'][$indicador->codigo]['RI']['nota']; 
								if ($r1==""){
