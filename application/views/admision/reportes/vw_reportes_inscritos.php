				...
				<div class="row">
					<div class="col-5">
						<div class="form-check">
							<input class="form-check-input rd_reportes" data-title='Inscripciones - Documentos Anexados' type="radio" data-view='ts_div_vwreportes' data-urlpdf='' data-urlexcel='matriculas/reportes/excel/documentos-anexados' name="exampleRadios" id="r303" value="303" checked>
							<label class="form-check-label" for="r303">
								Matrículas - Documentos Anexados
							</label>
						</div>
						<div class="form-check">
							<input class="form-check-input rd_reportes" data-title='Inscripciones - Documentos Anexados' type="radio" data-view='ts_div_vwreportes' data-urlpdf='admision/inscripcion-reportes/pdf' data-urlexcel='admision/inscripcion-reportes/excel' name="exampleRadios" id="r301" value="301">
								...
								<form id="frm_search_inscritos" action="<?php echo base_url() ?>inscrito_reportes/get_filtrar_basico_sd_activa_report" method="post" accept-charset="utf-8">
									<div class="row my-2">
						                ...
						                <div class="form-group has-float-label col-12 col-sm-4 col-md-4">
						                  	<select data-currentvalue="" class="form-control" id="fbus-ciclo" name="fbus-turno" placeholder="Semestre" required="">
						                    	<option value="%">Todos</option>
	...
	$("#fbus-periodo").change(function(event) {
  	...
  	$('#vw_exp_excel').click(function(e) {
    	...
    	if (llenos>1) {
    		...
    		var campania = $("#fbus-campania").val();
    		var seccion = $("#fbus-seccion").val();
    		var ciclo = $("#fbus-ciclo").val();
    		var carrera = $("#fbus-carrera").val();
    		var turno = $("#fbus-turno").val();
    		var url_pdf = $("#divcard_ls_reportes input[type='radio']:checked").data('urlexcel');

    		var url = base_url + url_pdf + '?bc=' + buscar + '&per=' + periodo + '&cp=' + campania + '&sc=' + seccion + '&pg=' + carrera + '&cc=' + ciclo + '&tn=' + turno;
    		window.open(url, '_blank');
	      	$("#divres-historial").html("");
	    }
    	else{
