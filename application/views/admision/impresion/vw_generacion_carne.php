...
<script>
	...
	$('#btn_addbandeja').click(function(e) {
		...
		nrocarnes = tbmatriculadosbndj.rows().count();

		if (bandejatb > 0) {
			nropag = nrocarnes / 5;
			$('#divcard_text_Cantidad').html(nrocarnes + ' Carnés en lista (' + Math.ceil(nropag) + 'páginas)');
		} else {
	...
	$('#btn_deletebandeja').click(function(e) {
		...
		$('#divcard_tbbandeja #preview_foto_estudiante').html('');
		...
		bandejatb = $("#content_bandeja .cfila_mt").length;
		nrocarnes = tbmatriculadosbndj.DataTable().rows().count();
		if (bandejatb > 0) {
			nropag = nrocarnes / 5;
			$('#divcard_text_Cantidad').html(nrocarnes + ' Carnés en lista (' + Math.ceil(nropag) + 'páginas)');
		} else {
	...
	$('#btn_impresion').click(function(e) {
	    ...
	    if (bandejatb > 0) {
	    	arraycarne = [];
	    	tbmatriculadosbndj.rows().iterator('row', function (context, index) {
			    // var node = $(this.row(index).node());
			    var fila = $(this.row(index).node());
	...
	function fn_viewfoto(btn) {
		...
		if (vmatriculas > 0) {
			if (row.hasClass('selected')) {
				...
				$('#modupdatefoto #btn_guardarfoto').data('ficfila', row);
				$('#modupdatefoto #btn_guardarfoto').data('insc', codins64);
			} else {
				Swal.fire({
	                type: 'warning',
	                icon: 'warning',
