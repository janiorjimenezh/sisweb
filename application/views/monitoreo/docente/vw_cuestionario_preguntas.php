...
function echocard($vnum,$vclase,$tipospg,$pgta,$resp){
        ...
        echo "<div class='card-pregunta $vclase card  ml-sm-2 ml-md-3 card-outline $colorcard' data-numero='$vnum' >
                ...
                <div class='card-header px-2 px-sm-3 py-2 text-sm'>
                    <h4 class='card-title text-bold'>Pregunta $vnum</h4>
                ...
                <div class='card-body px-2 px-sm-3 py-2'>
                    ...
                    echo "</div>
                        <div class='col-1 d-none d-sm-block'>
                                ...
                                <a  href='#' class='btn-aseccion btn btn-sm btn-outline-secondary m-1' onclick='clonepreg($(this));event.preventDefault();'>
...
function clonepreg(btn) {
    //$('.clcheckmostrar').bootstrapToggle('destroy');
    var card = btn.closest(".card-pregunta.pregunta");
    var codclone=card.find("[name='pg-codpg']").val();
    //alert(codclone);
    card.append('<div id="divoverlay" class="overlay dark"><i class="fas fa-spinner fa-pulse fa-3x"></i></div>');
    ...
    //Inicializar Clon
    newcard.find("[name='pg-codpg']").val(0);
    
    newcard.find(".rpta-radio").each(function() {
    ...
    newcard.find('textarea').removeClass('is-invalid');
    newcard.find('.invalid-feedback').remove();
    //////////////////////////////////////////////////
    //var newcard= btn.closest(".card-pregunta.pregunta");
    
    //Clonamos
    //alert("ddd");
    $.ajax({
        url: base_url + 'cuestionario_general/fn_clonar_pegunta',
        type: 'POST',
        data: {
                "codclone": codclone
            },
        ...
        success: function(e) {
            ...
            if (e.status == true) {
                newcard.find("[name='pg-enunciado']").focus();
                /*if (e.newid!=="--"){
                    newcard.find("[name='pg-codpg']").val(e.newid);
                }
                newcard.find('input,textarea,select').each(function() {
                    $(this).data('currentvalue', $(this).val());
                });
                var cheh=newcard.find("[name='pg-obligatoria']");
                trad=(cheh.prop('checked')==true)?"SI":"NO";
                cheh.data('currentvalue', trad);
                //frmdp_edit[crd] = new Array();
                //newcard.find(".vw_aep_editp").html("");

                newcard.removeClass('card-danger');
                newcard.addClass('card-primary');
