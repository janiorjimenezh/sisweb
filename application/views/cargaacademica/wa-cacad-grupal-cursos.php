		...
		<?php 
			foreach ($divisiones as $key => $divi) {
				if ($carga->codcarga==$divi->codcarga){
					...
					?>
					<div class="row fdivision bg-white" data-codcarga64='<?php echo $pcodcarga_enc ?>' data-division64='<?php echo $pcoddivision_enc ?>' data-coddivision='<?php echo $pcoddivision_enc ?>' data-codunidad64='<?php echo $codunidad64 ?>'  data-coddocente='<?php echo $pcoddocente_enc ?>'  data-coddoc='<?php echo $divi->coddocente ?>' data-unidad='<?php echo "$carga->unidad ($carga->codunidad)" ?>'>
						<div class="col-4 col-md-2 cell text-center">
							<div class="row border-top-0 border-bottom-0">
								<div class="col-12 col-md-12">
									<?php
									if (getPermitido("212") == "SI") {
										echo "<a class='fd-eliminardivision text-danger' href='#' title='Eliminar División' data-grupo='$divi->division' data-carga='$divi->codcarga'><i class='fas fa-minus-square mr-1 '></i></a> ";
									}
									?>
