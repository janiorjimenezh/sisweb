          ...
          if (e.status==false){
              ...
              $.each(e.vdata, function(index, v) {
                ...
                var params='cp='+ v['codperiodo'] +'&cc='+ v['codcarrera'] + '&ccc='+ v['codciclo'] +'&ct='+ v['turno']+'&cs='+ v['seccion']+'&cpl='+ v['idplan'];
                 ...
                 btnord_merito='<div class="td col-2 col-md-2 text-right text-primary"><a title="Orden de mérito" class="bg-info text-white py-1 px-2 mt-2 rounded" target="_blank" href="' + base_url + 'academico/consulta/orden-merito/imprimir?' + params + '&at=1'+'"><i class="fas fa-user-graduate"></i><i class="fas fa-sort-numeric-up-alt"></i> Mérito</a></div>' ;
                if (per_vcxg=='SI'){
                  
                  btn_carga='<div class="td col-2 col-md-2 text-right text-primary"><a title="Carga Académica" class="bg-primary text-white py-1 px-2 mt-2 rounded" target="_blank" href="' + base_url + 'gestion/academico/carga-academica/grupo?' + params +'"><i class="fas fa-book-open"></i> Carga</a></div>' ;
                }
                if (per_vmxg=='SI'){
                  //var params='cp='+ v['codperiodo'] +'&cc='+ v['codcarrera'] + '&ccc='+ v['codciclo'] +'&ct='+ v['turno']+'&cs='+ v['seccion']+'&cpl='+ v['idplan'];
                  btn_matriculas='<div class="td col-2 col-md-2 text-right text-primary"><a title="Listar matriculados" class="bg-primary text-white py-1 px-2 mt-2 rounded" target="_blank" href="' + base_url + 'gestion/academico/matriculas?' + params + '&at=1'+'"><i class="fas fa-user"></i> Matrículas</a></div>' ;
                }
                 btn_matricular='<div class="td col-2 col-md-2 text-right text-primary">' +
                 '<a data-periodo=' + v['codperiodo'] + ' data-carrera=' + v['codcarrera'] + ' data-ciclo=' + v['codciclo'] + ' data-turno=' + v['turno'] + ' data-seccion=' + v['seccion'] + ' data-plan=' + v['idplan'] + ' title="Agregar matrícula" class="bg-success text-white py-1 px-2 mt-2 rounded"  href="#" data-toggle="modal" data-target="#modal-addalumno"><i class="fas fa-user-plus"></i> Matricular</i></a></div>' ;

                 btn_egresar='<div class="td col-2 col-md-2 text-right text-primary">' +
                 '<a data-periodo=' + v['codperiodo'] + ' data-carrera=' + v['codcarrera'] + ' data-ciclo=' + v['codciclo'] + ' data-turno=' + v['turno'] + ' data-seccion=' + v['seccion'] + ' data-plan=' + v['idplan'] + ' title="Agregar matrícula" class="bg-success text-white py-1 px-2 mt-2 rounded"  href="#" data-toggle="modal" data-target="#modal-addalumno"><i class="fas fa-user-graduate"></i> Egresar</i></a></div>' ;

                $("#div-filtro").append(
                '<div class="row mb-4 pb-2 ' + rowcolor + ' ">' + 
                  '<div class="col-12 col-md-2">' + 
                    '<div class="row">' + 
                      '<div class="col-3 col-md-4 td">' + nro + '</div>' +
                      '<div class="col-9 col-md-8 td">' + v['periodo'] + '</div>' + 
                      
                    '</div>' + 
                  '</div>' + 
                  '<div class="col-8 col-md-3 td">' + v['carrera'] + '</div>' +
                  '<div class="col-6 col-md-4">' + 
                    '<div class="row">' + 
                      '<div class="col-7 col-md-7 td" title="' + v['idplan']  +'">' + v['plan'] + '</div>' + 
                      '<div class="col-5 col-md-5 td">' + v['ciclo'] + " - <b>" + v['turno'] + "</b> - " + v['seccion'] + '</div>' + 
                    '</div>' + 
                  '</div>' + 
                  ...
                  '<div class="col-12 col-md-9 ">' + 
                    '<div class="row">' + 
                      '<div class="col-4 col-md-2 td text-right">' + 
                        '<div class="btn-group btn-group-sm btn-secondary p-0 border-0" role="group">' + 
                        ...
                        '</div>' +
                      '</div>' +
                        btn_carga +  btn_matriculas + btn_matricular +   btnord_merito +  btn_egresar + 
                    '</div>' + 
                  '</div>' + 
                  '<div class="d-sm-none d-md-block col-md-3 td ">' + 
                  '</div>' +  
                '</div>');
