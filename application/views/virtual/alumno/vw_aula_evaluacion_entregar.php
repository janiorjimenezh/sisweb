                                    ...
                                    foreach ($resp as $key => $rp) {
                                        ...
                                        "<div class='rpta-radio $colrp' data-idrp='$codrp' >
                                            <div class='input-group input-group-sm mb-1'>
                                              ...
                                              <span class='form-control h-auto'>".htmlspecialchars($rpenunciado)."</span>
...
$(document).ready(function() {
    ...
    var status_miembro = '<?php echo $statusmiembro ?>';
    if (status_miembro == 'NO') {
        var vcodeval = $('#vidmaterial').val();
        var vcodmiembro = $('#vcodmiembro').val();
        var vcoddivision= $('#vdivision').val();
        var vcodcarga= $('#vidcurso').val();
        Swal.fire({
            title: 'Advertencia!',
            html: '<p>Este enlace no le pertenece.</p>Pertenece a otro estudiante',
            // text: ''
            type: 'warning',
            ...
            if (result.value) {
                location.href = base_url + "alumno/curso/virtual/" + vcodcarga + "/" + vcoddivision + "/" + vcodmiembro;
            }
        })
    }
});
            ...
            $.ajax({
                ...
                success: function(e) {
                    ...
                    if (e.status == true) {
                        ...
                        fn_view_examen_enviado(codcarga,coddivision,codeval,codmiembro);
...
function fn_view_examen_enviado(carga,division,material,miembro) {
    $('#divcard_evamensaje').append('<div id="divoverlay" class="overlay"><i class="fas fa-spinner fa-pulse fa-3x"></i></div>');
    $.ajax({
        url: base_url + "virtualevaluacion/vw_evaluaciones_individual_respuestas",
        type: "post",
        dataType: 'json',
        data: 
        {
        ...
        success: function(e) {
            ...
            if (e.status == false) {
                Swal.fire({
                    title: "Error!",
                    text:  "No se pudo mostrar el examen",
                    icon: 'error',
                })
            } else {
                $('#divcard_responde').html(e.vieweval);
