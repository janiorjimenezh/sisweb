...
class Inscritos extends CI_Controller {
	...
	public function index($modalidad="inscripciones"){
		...
		$asidebar= array('menu_padre' =>'estudiantes','menu_hijo' =>"est-inscripciones");
		...
		if (getPermitido("45")=='SI'){
			...
			$this->load->view('academico/estudiantes/vw_inscripciones',$a_ins);
		}
		else{
	...
	public function egresados($modalidad="egresados"){
		$modalidadActiva=$modalidad;
	...
	public function fnFiltrarInscripciones(){
		...
		if ($this->input->is_ajax_request())
		{
			...
			else
			{
				...
				$convalida=$this->input->post('fbus-convalida');
				$busqueda="%".str_replace(" ","%",trim($busqueda))."%";
       
				$cuentas = $this->minscrito->mFiltrarInscripciones(array('codperiodo'=>$periodo,'codmodalidad'=>$modalidad,'codcampania'=>$campania,'codcarrera'=>$carrera,'codciclo'=>$ciclo,'codturno'=>$turno,'codseccion'=>$seccion,'codsede'=>$sede,'convalida'=>$convalida,'estudiante'=>$busqueda));
