...
class Reingresos extends CI_Controller {
	...
	public function fn_insert_reingreso()
	{
		...
		if ($this->input->is_ajax_request())
		{
			...
			$this->form_validation->set_rules('ficbseccion','seccion','trim|required');
			...
			else
			{
				...
				$newcod=$this->minscrito->m_insert_reingreso(array($fimcid,$ficbmodalidad,$ficbperiodo,$sede,$ficbcampania,$ficbciclo,$fitxtobservaciones,$fecharegistro,$fitxtfecinscripcion,'0',$usuario,$ficbturno,$ficbseccion));
				...
				if ($newcod->salida == 1){
					...
					$rsrow=$this->mmatricula->m_insert(array($newcod->nid,"O","1",$ficbperiodo,$ficbcarrera,$ficbciclo,$ficbturno,$ficbseccion,$fmtxtcuota,6,$fitxtfecinscripcion,"","0",$fmtapepaterno,$fmtapematerno,$fmtnombres,$fmtsexo,$sede,$fmtxtcuotareal,$usuario,null,null,null,null));
